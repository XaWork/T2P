...
class ShopActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        } else if (type != null && intent.getStringExtra("type")!!.contentEquals("Flavours Of India")) {
            Log.e("shop", "cuisine")
    ...
    private fun productsByCuisine(cuisineId: String) {
        ...
        val filter = ProductFilter()
        filter.setPer_page(100)
        viewModel.productsByCuisine(cuisineId).observe(this, androidx.lifecycle.Observer { response ->
            when (response!!.status()) {
                Status.LOADING -> {
                    showLoading()
                }
